        ...
        _UriList ->
            ...
            case nksip_sdp:is_sdp(Body) of
                true ->
                    ...
                    Contact = case Contacts of
                                  [C | _] -> C;
                                  _ -> undefined
                              end,
                    if Contact =/= undefined ->
                        %% Сохраняем соответствие FromUser -> полный SIP URI
                        ets:insert(sip_uri_cache, {FromUser, Contact}),
                        io:format("Saved mapping: ~p -> ~p~n", [FromUser, Contact]);
                       true ->
                        ok
                    end,
