%%%-------------------------------------------------------------------
%% @doc sip_call_http top level supervisor.
%% @end
%%%-------------------------------------------------------------------

-module(sip_call_http_sup).
...
init([]) ->
    ...
    io:format("sip_call_http_sup: ChildSpecs ~p~n", [ChildSpecs]),
    SupFlags = #{
...
dump() ->
    io:format("++++ Supervisor tree: sip_call_http_sup ~p~n", [whereis(sip_call_http_sup)]),
    print_tree(whereis(sip_call_http_sup), sip_call_http_sup, 0),
